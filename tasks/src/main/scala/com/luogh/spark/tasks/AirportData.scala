...
object AirportData {
  ...
  def run(args: TravelSparkParam)(sc: => SparkContext): Unit = {
    ...
    datas.registerTempTable("airport_tab")
    ...
    val resultRDD = sqlContext.sql("select tmp_tdid.tdid, tmp_tdid.sceneryId, tmp_air.sentTime, " +
      ...
      "airport_tab AS tmp_air ON tmp_tdid.tdid = tmp_air.tdid")
      .rdd.flatMap(r => {
      ...
      r.getAs[mutable.WrappedArray[Long]]("sentTime").map { x =>
        ...
        (tdid, scneneryId, 0, date._1, id, poiid, name, address, lat, lng, province, city, district, date._2)
      }
